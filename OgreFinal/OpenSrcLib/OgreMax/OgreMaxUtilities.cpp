...
void OgreMaxUtilities::LoadSubentities(const TiXmlElement* objectElement, std::vector<EntityParameters::Subentity>& subentities)
{
    ...
    while ((childElement = IterateChildElements(objectElement, childElement)))
    {
		...
		subentities[index].renderQueue = GetStringAttribute(childElement, "renderQueue");
		subentities[index].renderQueuePriority = (Ogre::ushort)GetUIntAttribute(childElement, "renderQueuePriority", 0);
    }
}
...
int OgreMaxUtilities::GetUIntAttribute(const TiXmlElement* xmlElement, const char* name, unsigned int defaultValue, bool* retrieved)
{
	Ogre::String value = GetStringAttribute(xmlElement, name);
	if (value.empty())
	{
	...
	else
	{
		if (retrieved != 0)
			*retrieved = true;
		return Ogre::StringConverter::parseUnsignedInt(value);
	}
}
    ...
    )
{
	...
	#if OGRE_VERSION_MAJOR >= 1 && OGRE_VERSION_MINOR >= 8 && OGRE_VERSION_PATCH >= 1
		if (!subentities[subentityIndex].renderQueue.empty())
		{
			Ogre::uint8 renderQueue = ParseRenderQueue(subentities[subentityIndex].renderQueue);
			subentity->setRenderQueueGroupAndPriority(renderQueue, subentities[subentityIndex].renderQueuePriority);
		}
	#endif
    }
...
bool OgreMaxUtilities::SetDefaultLighting(Ogre::SceneManager* sceneManager, UpAxis upAxis)
{
    ...
    if (defaultLightingNeeded)
    {
        ...
        else if (sceneManager->getAmbientLight() == Ogre::ColourValue::White)
...
{
	//Stick with a basic collection of formats
	return 
		name == "bmp" ||
		name == "jpg" ||
		name == "png"
