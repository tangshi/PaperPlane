 ...
 * Copyright 2013 AND Entertainment
 *
...
void OgreMaxModel::LoadNode(const TiXmlElement* objectElement, NodeParameters& node)
{
    ...
    while ((childElement = OgreMaxUtilities::IterateChildElements(objectElement, childElement)))
    {
        ...
        {
            extraData.noteTracks = Ogre::SharedPtr<NoteTracks>(new NoteTracks);
...
EntityParameters* OgreMaxModel::LoadEntity(const TiXmlElement* objectElement)
{
	...
	parameters->queryFlags = OgreMaxUtilities::GetUIntAttribute(objectElement, "queryFlags", 0, &parameters->queryFlagsEnabled);
	parameters->visibilityFlags = OgreMaxUtilities::GetUIntAttribute(objectElement, "visibilityFlags", 0, &parameters->visibilityFlagsEnabled);
    ...
    Ogre::String skeletonAnimationBlendMode = OgreMaxUtilities::GetStringAttribute(objectElement, "skeletonAnimationBlendMode");
    ...
    Ogre::String renderQueue = OgreMaxUtilities::GetStringAttribute(objectElement, "renderQueue");
...
LightParameters* OgreMaxModel::LoadLight(const TiXmlElement* objectElement)
{
    ...
    Ogre::String type = OgreMaxUtilities::GetStringAttribute(objectElement, "type", "point");
...
CameraParameters* OgreMaxModel::LoadCamera(const TiXmlElement* objectElement)
{
    ...
    parameters->fov = Ogre::Radian(OgreMaxUtilities::GetRealAttribute(objectElement, "fov", Ogre::Math::PI/2));
    
    parameters->aspectRatio = OgreMaxUtilities::GetRealAttribute(objectElement, "aspectRatio", (Ogre::Real)1.33);

    parameters->orthoWidth = OgreMaxUtilities::GetRealAttribute(objectElement, "orthoWidth", (Ogre::Real)50);
    parameters->orthoHeight = OgreMaxUtilities::GetRealAttribute(objectElement, "orthoHeight", (Ogre::Real)50);

    Ogre::String projectionType = OgreMaxUtilities::GetStringAttribute(objectElement, "projectionType", "perspective");
...
BillboardSetParameters* OgreMaxModel::LoadBillboardSet(const TiXmlElement* objectElement)
{
    ...
    Ogre::String origin = OgreMaxUtilities::GetStringAttribute(objectElement, "origin", "center");
    parameters->origin = OgreMaxUtilities::ParseBillboardOrigin(origin);

    Ogre::String rotationType = OgreMaxUtilities::GetStringAttribute(objectElement, "rotationType", "vertex");
...
void OgreMaxModel::LoadBillboard(const TiXmlElement* objectElement, BillboardSetParameters::Billboard& billboard)
{
    ...
    billboard.rotationAngle = Ogre::Radian(OgreMaxUtilities::GetRealAttribute(objectElement, "rotation", 0));
    ...
    while ((childElement = OgreMaxUtilities::IterateChildElements(objectElement, childElement)))
    {
        ...
        {
            Ogre::Quaternion rotation = OgreMaxUtilities::LoadRotation(childElement);
...
{
    if (light.lightType == Ogre::Light::LT_SPOTLIGHT)
    {
        Ogre::String value;

        value = OgreMaxUtilities::GetStringAttribute(objectElement, "inner");
        if (!value.empty())
            light.spotlightInnerAngle = Ogre::Radian(Ogre::StringConverter::parseReal(value));

        value = OgreMaxUtilities::GetStringAttribute(objectElement, "outer");
        if (!value.empty())
            light.spotlightOuterAngle = Ogre::Radian(Ogre::StringConverter::parseReal(value));

        value = OgreMaxUtilities::GetStringAttribute(objectElement, "falloff");
        if (!value.empty())
            light.spotlightFalloff = Ogre::StringConverter::parseReal(value);
    }
}
...
void OgreMaxModel::LoadLightAttenuation(const TiXmlElement* objectElement, LightParameters& light)
{
    ...
    if (!value.empty())
        light.attenuationRange = Ogre::StringConverter::parseReal(value);

    value = OgreMaxUtilities::GetStringAttribute(objectElement, "constant");
    if (!value.empty())
        light.attenuationConstant = Ogre::StringConverter::parseReal(value);

    value = OgreMaxUtilities::GetStringAttribute(objectElement, "linear");
    if (!value.empty())
        light.attenuationLinear = Ogre::StringConverter::parseReal(value);

    value = OgreMaxUtilities::GetStringAttribute(objectElement, "quadric");
    if (!value.empty())
        light.attenuationQuadric = Ogre::StringConverter::parseReal(value);
}
...
void OgreMaxModel::LoadNodeAnimation(const TiXmlElement* objectElement, NodeAnimationParameters& animation)
{
    ...
    //Interpolation mode
    Ogre::String interpolationModeText = OgreMaxUtilities::GetStringAttribute(objectElement, "interpolationMode");
    ...
    Ogre::String rotationInterpolationModeText = OgreMaxUtilities::GetStringAttribute(objectElement, "rotationInterpolationMode");
    ...
    ) const
{
    ...
    if (!nodeParams.animations.empty())
    {
        for (size_t animationIndex = 0; animationIndex < nodeParams.animations.size(); animationIndex++)
        {
            ...
            Ogre::String animationName = baseName + animationParams.name;
...
{
    Ogre::String objectName = baseName + object->name;
    ...
    ) const
{
    ...
    //Create entity
    Ogre::Entity* entity = sceneManager->createEntity(objectName, entityParams->meshFile);
	if (entityParams->queryFlagsEnabled)
		entity->setQueryFlags(entityParams->queryFlags);
    if (entityParams->visibilityFlagsEnabled)
...
{
    //Create the light
    Ogre::Light* light = sceneManager->createLight(objectName);
    if (lightParams->queryFlagsEnabled)
		light->setQueryFlags(lightParams->queryFlags);
    if (lightParams->visibilityFlagsEnabled)
    ...
    light->setSpotlightInnerAngle(Ogre::Radian(lightParams->spotlightInnerAngle));
    light->setSpotlightOuterAngle(Ogre::Radian(lightParams->spotlightOuterAngle));
	...
	if (lightParams->direction == Ogre::Vector3::ZERO)
...
{
    //Create the camera
    Ogre::Camera* camera = sceneManager->createCamera(objectName);
    if (cameraParams->queryFlagsEnabled)
		camera->setQueryFlags(cameraParams->queryFlags);
    if (cameraParams->visibilityFlagsEnabled)
    ...
    if (cameraParams->direction == Ogre::Vector3::ZERO)
    ...
    ) const
{
    ...
    Ogre::ParticleSystem* particleSystem = sceneManager->createParticleSystem(objectName, particleSystemParams->file);
    if (particleSystemParams->queryFlagsEnabled)
		particleSystem->setQueryFlags(particleSystemParams->queryFlags);
    if (particleSystemParams->visibilityFlagsEnabled)
    ...
    ) const
{
    ...
    Ogre::BillboardSet* billboardSet = sceneManager->createBillboardSet(objectName);
    if (billboardSetParams->queryFlagsEnabled)
		billboardSet->setQueryFlags(billboardSetParams->queryFlags);
    if (billboardSetParams->visibilityFlagsEnabled)
    ...
    for (size_t billboardIndex = 0; billboardIndex < billboardSetParams->billboards.size(); billboardIndex++)
    {
        ...
        Ogre::Billboard* billboard = billboardSet->createBillboard(billboardParams.position, billboardParams.color);
...
{
    //Create plane mesh
    Ogre::String planeName = baseName + planeParameters->planeName;
    ...
    //Create plane entity
    Ogre::Entity* entity = sceneManager->createEntity(objectName, planeName);
    if (planeParameters->queryFlagsEnabled)
		entity->setQueryFlags(planeParameters->queryFlags);
    if (planeParameters->visibilityFlagsEnabled)
