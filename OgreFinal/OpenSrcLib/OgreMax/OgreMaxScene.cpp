...
public:
	...
	virtual void ReferencedNodeAnimationState(const OgreMaxModel* model, SceneNode* node, AnimationState* animationState)
    {
        if (this->sceneCallback != 0)
            this->sceneCallback->ReferencedNodeAnimationState(this->scene, node, animationState);
    }
...
void OgreMaxScene::LoadEntity(const TiXmlElement* objectElement, const MovableObjectOwner& owner)
{
    ...
    if (parameters.queryFlagsEnabled)
		entity->setQueryFlags(parameters.queryFlags);
	if (parameters.visibilityFlagsEnabled)
...
void OgreMaxScene::LoadLight(const TiXmlElement* objectElement, const MovableObjectOwner& owner)
{
    ...
    objectExtraData->id = OgreMaxUtilities::GetStringAttribute(objectElement, "id");
	bool queryFlagsEnabled, visibilityFlagsEnabled;
	uint32 queryFlags = OgreMaxUtilities::GetUIntAttribute(objectElement, "queryFlags", 0, &queryFlagsEnabled);
	uint32 visibilityFlags = OgreMaxUtilities::GetUIntAttribute(objectElement, "visibilityFlags", 0, &visibilityFlagsEnabled);
    ObjectVisibility visibility = OgreMaxUtilities::GetObjectVisibilityAttribute(objectElement, "visible");
	...
	objectExtraData->affectDiffuse = OgreMaxUtilities::GetBoolAttribute(objectElement, "affectDiffuse", true);
	objectExtraData->affectSpecular = OgreMaxUtilities::GetBoolAttribute(objectElement, "affectSpecular", true);
	objectExtraData->ambientOnly = OgreMaxUtilities::GetBoolAttribute(objectElement, "ambientOnly", false);
	
    //Create the light
    Light* light = this->sceneManager->createLight(name);
    if (queryFlagsEnabled)
		light->setQueryFlags(queryFlags);
    if (visibilityFlagsEnabled)
...
void OgreMaxScene::LoadNodeAnimation(const TiXmlElement* objectElement, SceneNode* node)
{
    ...
    if ((this->loadOptions & NO_ANIMATION_STATES) == 0)
    {
		...
		AnimationState* animationState = GetAnimationState(params.name);
        if (animationState == 0)
        {
